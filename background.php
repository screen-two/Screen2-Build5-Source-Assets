...
if(intval($trend_limit) > 0) {
	//limit trends to Ireland
	$trends = $connection->get("https://api.twitter.com/1.1/trends/place.json?id=23424803");
	$trends = $trends[0];
	
	$test = $trends->{'trends'};
	
	foreach ($test as $trend){
		...
		if(empty($result)){  
			//insert search
			echo "adding trend:" . $q . "<br/>";
			$query = mysql_query(sprintf("INSERT INTO saved_search ( search_terms, last_tweet_id ) VALUES ('%s', '%s')", mysql_real_escape_string($q), ''));  
		}
	}
}
